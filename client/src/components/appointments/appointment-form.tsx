...
const appointmentSchema = z.object({
  ...
  procedureId: z.number().min(1, "Procedimento é obrigatório"),
...
export default function AppointmentForm({ appointment, onSuccess, onCancel }: AppointmentFormProps) {
  ...
  const form = useForm<AppointmentFormData>({
    ...
    defaultValues: {
      ...
      procedureId: appointment?.procedureId || 0,
    ...
    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          ...
          <Select 
            value={form.watch("procedureId")?.toString() || ""} 
            onValueChange={(value) => form.setValue("procedureId", parseInt(value))}
          >
            <SelectTrigger>
            ...
            <SelectContent>
              {procedures?.map((procedure) => (
                <SelectItem key={procedure.id} value={procedure.id.toString()}>
                  {formatProcedureOption(procedure)}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
          {form.formState.errors.procedureId && (
            <p className="text-sm text-red-600">{form.formState.errors.procedureId.message}</p>
          )}
        </div>
      ...
      {/* Appointment Summary */}
      {form.watch("procedureId") && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <h4 className="text-sm font-medium text-blue-900 mb-2">Resumo do Agendamento</h4>
          {(() => {
            const selectedProcedure = procedures?.find(p => p.id === form.watch("procedureId"));
            ...
            return (
              <div className="text-sm text-blue-800 space-y-1">
                ...
                <p><strong>Procedimento:</strong> {selectedProcedure?.name || "Não selecionado"}</p>
                {selectedProcedure && (
                  <>
                    <p><strong>Duração:</strong> {
                      selectedProcedure.duration >= 60 
                        ? `${Math.floor(selectedProcedure.duration / 60)}h${selectedProcedure.duration % 60 > 0 ? ` ${selectedProcedure.duration % 60}min` : ''}`
                        : `${selectedProcedure.duration}min`
                    }</p>
                    <p><strong>Valor:</strong> R$ {Number(selectedProcedure.price).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                  </>
                )}
